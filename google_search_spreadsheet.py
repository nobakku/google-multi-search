...
# Googleスプレッドシート使用のための設定
# 2つのAPIを記述しないとリフレッシュトークンを3600秒毎に発行し続けなければならない
scope = ['https://www.googleapis.com/auth/spreadsheets','https://www.googleapis.com/auth/drive']
# 認証情報設定
# ダウンロードしたjsonファイル名をクレデンシャル変数に設定（秘密鍵、Pythonファイルから読み込みしやすい位置に置く）
credentials = Credentials.from_service_account_file("calm-velocity-376608-8fcc97272755.json", scopes=scope)
...
def get_info(driver, keyword):
    ...
    # seleniumで検索結果のurlの取得（1位から10位までのURLのa要素をリストの形で取得）
    urls = driver.find_elements(By.CSS_SELECTOR, 'div.Z26q7c.UK95Uc.jGGQ5e > div > a')
    """
    Google検索で「◯◯」と調べ､URLを検証でみて､ aタグのCopy selector をしたときに出てくる上位3つ｡
    「div.Z26q7c.UK95Uc.jGGQ5e > div > a」以降が共通｡
    検証 → aタグ選択 → コピー → outerHTMLをコピー で取得できるHTMLを自動取得
    """

    # urlの有無確認
    if urls:
        for url in urls:
            # 上の辞書からKey([])を指定して､Keyに.append(url.get_attribute('href')) でURLだけを取得し両端の空白削除した上で追加
            items['url'].append(url.get_attribute('href').strip())

    # titleの取得
    titles = driver.find_elements(By.CSS_SELECTOR, 'div.Z26q7c.UK95Uc.jGGQ5e > div > a > h3')
    if titles:
        for title in titles:
            items['title'].append(title.text.strip())
    # descriptionの取得
    descriptions = driver.find_elements(By.CSS_SELECTOR, 'div.Z26q7c.UK95Uc > div.VwiC3b.yXK7lf.MUxGbd.yDYNvb.lyLwlc.lEBKkf > span')
    if descriptions:
    ...
    # items['url']の中のh1~h5見出しの取得
    # urlを一個一個取り出して､SSlエラーの有無でtry・except文で条件分岐させる
...
def spread_out(items):
    ...
    # シートが作成されているか確認するためのフラグ エラー時の場合分けで使用
    flag = False

    try:
        # 共有設定したスプレッドシートのシート1を開く
        workbook = gc.open_by_key(local_config.SPREADSHEET_KEY)
